...
# ----------------------------
# MCNet Implementation (VANet + CANet)
# ----------------------------
...
class CANet(nn.Module):
    def __init__(self):
        ...
        self.regular_conv = nn.Conv2d(11, 32, kernel_size=3, padding=1, dilation=1)
        self.bt1 = BT1(64, 64)
        self.bt2_blocks = nn.Sequential(
            BT2(64), BT2(64), BT2(64), BT2(64)
        )
        self.bt3_blocks = nn.Sequential(
            BT3(64, 128), BT3(128, 256), BT3(256, 256), BT3(256, 256)
        )
    def forward(self, x):
...
class MCNet(nn.Module):
    ...
    def forward(self, x_spatial, x_freq, x_compression, return_logits=False):
...
class FaceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        comp_feature = torch.from_numpy(comp_feature).float()
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs=10, device='cuda:1'):
    best_acc = 0.0
    start_epoch = 0
    checkpoint_path = 'mcnet_model_checkpoint.pth'
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for spatial, freq, comp, labels in tqdm(train_loader, desc=f'Epoch {epoch+1} [Train]'):
        ...
        val_corrects = 0
        with torch.no_grad():
            for spatial, freq, comp, labels in tqdm(val_loader, desc=f'Epoch {epoch+1} [Val]'):
                spatial, freq, comp, labels = spatial.to(device), freq.to(device), comp.to(device), labels.unsqueeze(1).to(device)
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), 'best_model_mcnet.pth')
...
if __name__ == "__main__":
    ...
    train_df = create_dataframe(root_dir, datatype='train')
